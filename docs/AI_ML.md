...
Kluczowe pliki kodu backendu to: `app/services/personas/persona_details_service.py` (orchestrator, ~250 linii), `app/services/personas/persona_audit_service.py` (audit logging), `app/models/persona_audit.py` (audit log model), `app/schemas/persona_details.py` (Pydantic schemas), `app/api/personas/details.py` (GET /personas/{id}/details endpoint), oraz odpowiednia migracja Alembic.
